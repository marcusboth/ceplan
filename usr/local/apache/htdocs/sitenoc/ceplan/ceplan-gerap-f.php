...
if($libera == TRUE)
{
 ...
 for($f=0;$f<$contadorplantao;$f++)
 {
  ...
  //echo "SQL $sql<BR>";
  $resultado = mysql_query($sql)
   or die ("Não foi possível realizar a consulta ao banco de dados tabpla- $periodo $dia  $flag $numplant  $madrugaa  $madrugab  $manhaa  $manhab  $tardea  $tardeb  $noitea  $noiteb");
 } 
  ...
  if(!empty($insysop[$f]))
   {
    ...
    //echo "sql $sql<BR>";
    $resultado = mysql_query($sql)
     or die ("Não foi possível gravar no banco, provavel registro duplicado.");
   }
 }
 ...
 $resultado = mysql_query($sql)
 or die ("Não foi possível realizar a consulta ao banco de dados tabsal - $ultnumseq");
