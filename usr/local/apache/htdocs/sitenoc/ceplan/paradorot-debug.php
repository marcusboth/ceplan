...
while($limitloop<=3)
 {
 $modo="normal";
 $procurasql="SELECT * FROM tabord WHERE periodo='$mesini' AND qtferias='0' AND nplant = 0.5 AND saldomes = 3 AND saltot < -3 ORDER BY numseq limit $loop, 1;";
 $mesg_retorno[$vezes]=procuraproximo($modo,$procurasql,$periodoemaberto);
 $loop++; $limitloop++;
 if(!empty($mesg_retorno[$vezes]))
  {
...
while($limitloop<=3)
 {
 $modo="normal";
 $procurasql = "SELECT * FROM tabord WHERE periodo='$mesini' AND qtferias='0' AND nplant = 1 AND saltot <= 6 ORDER BY numseq limit $loop, 1";
 $mesg_retorno[$vezes]=procuraproximo($modo,$procurasql,$periodoemaberto);
 $loop++;  $limitloop++;
 if(!empty($mesg_retorno[$vezes]))
  {
